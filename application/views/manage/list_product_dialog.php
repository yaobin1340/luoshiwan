<form id="pagerForm" method="post" action="<?php echo site_url('manage/list_product_dialog').'/'.$term_id?>">
...
<div class="pageHeader" id="dialog">
	<form onsubmit="return dialogSearch(this);" action="<?php echo site_url('manage/list_product_dialog')?>" method="post">
		<div class="searchBar">
			...
			if (!empty($res_list)):
				foreach ($res_list as $row):
					...
					<tr target="id" rel=<?php echo $row->id; ?>>
						...
						<td><b><font color="<?php echo $row->rgb?>"><?php echo $row->color;?></font></b></td>
						<td><?php echo $row->cdate;?></td>
						<td>
							<a class="btnSelect" onclick="bringBack_current('<?php echo $row->num;?>','<?php echo $row->color;?>','<?php echo $row->rgb;?>');" href="javascript:;" >选择</a>
						</td>
					</tr>
					<?php
				endforeach;
...
<script>
	function bringBack_current(num,color,rgb){
		$.get('<?php echo site_url('manage/get_size')?>/' + num,function(data){
			data = JSON.parse(data);
			list_size(data,num,color,rgb);
