<form id="pagerForm" method="post" action="<?php echo site_url('manage/list_term')?>">
...
<div class="pageHeader">
	<form onsubmit="return navTabSearch(this);" action="<?php site_url('manage/list_term')?>" method="post">
	<div class="searchBar">
		<table class="searchContent" id="search_purchase_order">
			<tr>
				<td><label>专题名称：</label><input type="text" size="16" name="name" value="<?php echo $name;?>" /></td>
			</tr>
		</table>
		<div class="subBar">
			<ul>
...
<div class="pageContent">
	...
	<table class="list" width="100%" targetType="navTab" asc="asc" desc="desc">
		<thead>
			<tr>
				<th width="40%" orderField="name" <?php echo $this->input->post('orderField')=='name'?'class="'.$this->input->post('orderDirection').'"':'';?>>专题名称</th>
				<th width="45%" orderField="title" <?php echo $this->input->post('orderField')=='title'?'class="'.$this->input->post('orderDirection').'"':'';?>>标题</th>
				<th width="15%" orderField="created" <?php echo $this->input->post('orderField')=='created'?'class="'.$this->input->post('orderDirection').'"':'';?>>创建时间</th>
			</tr>
		</thead>
		<tbody>
            <?php          
