<form id="pagerForm" method="post" action="<?php echo site_url('manage/list_product_type')?>">
...
<div class="pageContent">
	<div class="panelBar">
		<ul class="toolBar">
			<li><a class="add" href="<?php echo site_url('manage/add_product_type')?>" target="dialog" rel="add_product_type" title="新建"><span>新建</span></a></li>
			<li><a class="delete" href="<?php echo site_url('manage/delete_product_type')?>/{id}" target="ajaxTodo"  title="确定要删除？" warn="请选择一条记录"><span>删除</span></a></li>
			<li><a class="edit" href="<?php echo site_url('manage/edit_product_type/{id}')?>" target="dialog" rel="add_product_type" warn="请选择一条记录" title="查看"><span>查看</span></a></li>
		</ul>
	</div>
	...
	<table class="list" width="100%" targetType="navTab" asc="asc" desc="desc">
		<thead>
			<tr>
				<th width="30" >lev</th>
				<th orderField="name" <?php echo $this->input->post('orderField')=='name'?'class="'.$this->input->post('orderDirection').'"':'';?>>标题</th>
				<th width="140" orderField="cdate" <?php echo $this->input->post('orderField')=='cdate'?'class="'.$this->input->post('orderDirection').'"':'';?>>创建时间</th>
				<th width="30" orderField="id" <?php echo $this->input->post('orderField')=='id'?'class="'.$this->input->post('orderDirection').'"':'';?>>ID</th>
			</tr>
		</thead>
		<tbody>
            <?php
            ...
            ?>
            			<tr target="id" rel="<?php echo $row['id'];?>"<?php if($row['lev'] == '1')echo 'style="color:blue;"';?> >
